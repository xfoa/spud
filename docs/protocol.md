...
| `FEATURE_ENCRYPT`  | `1 << 0`         | AES-256-GCM encryption for UDP data plane. |
| `FEATURES`         | `1 << 0`         | Feature bitmap advertised by this build. |
...
| `CTRL_KEY_EXCHANGE`| `0x06`           | Client session key.                      |
...
* `Features`: feature bitmap the client wishes to advertise. If the client has
  encryption enabled, it sets `FEATURE_ENCRYPT`. Otherwise it sends `0`.
...
* `Features`: negotiated feature bitmap, defined as
  `client.features & server.features`. If the client requires encryption but
  this value does not contain `FEATURE_ENCRYPT`, the client aborts the connection.
...
hash (or when auth is not required). The payload contains only the tag byte.

#### `0x06` KeyExchange (client -> server)

Sent by the client after `AuthAck` when `FEATURE_ENCRYPT` was negotiated.

```mermaid
packet-beta
title KeyExchange payload
0-7: "Tag = 0x06"
8-263: "Session key (32 bytes)"
```

* `Session key`: a random 32-byte value generated by the client. Both sides use
  it as the AES-256 key for encrypting UDP packets.
...
4. Client behavior after `HelloAck`:
   ...
   * If the client requires encryption but `Features` does not contain
     `FEATURE_ENCRYPT`, the client treats the connection as failed.
...
6. Client reads the server's reply. `AuthFailed` is reported as an authentication
   error in the UI.
7. If `FEATURE_ENCRYPT` was negotiated, the client generates a random 32-byte
   session key, sends it in a `CTRL_KEY_EXCHANGE` message, and remembers it for
   encrypting UDP packets. The server receives the key and stores it for the
   peer.
8. Client opens a UDP socket and `connect()`s it to the same `host:port`. UDP
...
When `FEATURE_ENCRYPT` is negotiated, every UDP datagram is encrypted with
AES-256-GCM before transmission. The plaintext is the raw event datagram
(tag + body). The on-wire format is:

```mermaid
packet-beta
title Encrypted UDP datagram
0-31: "Sequence number (u32 LE)"
32-..: "Ciphertext + auth tag (variable)"
```

* `Sequence number`: a monotonically increasing `u32` counter per packet. Used
  as the nonce: `seq.to_le_bytes() || [0u8; 8]` (12 bytes total).
* `Ciphertext + auth tag`: the output of AES-256-GCM encrypting the plaintext.
  The `aes-gcm` crate appends the 16-byte tag to the ciphertext.

On the server, if a session key exists for the peer IP, the server decrypts
using the same nonce construction before decoding the event. If no key exists,
the datagram is treated as plaintext (backward compatibility for peers that did
not negotiate encryption).
